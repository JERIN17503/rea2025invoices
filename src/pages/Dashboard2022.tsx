...
import { loadMasterlistClientData2022, PREMIUM_CLIENTS_2022, NORMAL_CLIENTS_2022, ONE_TIME_CLIENTS_2022 } from "@/lib/masterlistClientData2022";
import { loadMasterlistAggregates2022 } from "@/lib/masterlistAggregates2022";
import { ClientTable } from "@/components/ClientTable";
import { StatsCard } from "@/components/StatsCard";
import { InsightsPanel2022 } from "@/components/InsightsPanel2022";
...
const Dashboard2022 = () => {
  ...
  const revenueDenom = totals.totalRevenue || 1;
  const premiumSharePct = round1((totals.premiumTotal / revenueDenom) * 100);
  const normalSharePct = round1((totals.normalTotal / revenueDenom) * 100);
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs sm:text-sm text-muted-foreground">Invoice Masterlist 2022</p>
              </div>
            </div>
            <div className="flex items-center gap-2 overflow-x-auto pb-1 sm:pb-0">
              ...
              <div className="hidden sm:flex items-center gap-2 text-sm text-muted-foreground ml-2">
                ...
                <span>{totals.totalInvoices} invoices</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <main className="container mx-auto px-3 sm:px-4 py-4 sm:py-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 sm:gap-4 mb-6 sm:mb-8">
          <StatsCard
            ...
            subtitle={`${totals.totalInvoices} total invoices`}
            icon={Users}
          />
          <StatsCard
          ...
          <div className="bg-gradient-to-br from-premium/10 to-premium/5 rounded-xl border border-premium/20 p-4 sm:p-6">
            <div className="flex items-center justify-between mb-2">
              ...
              <ExportDialog
                ...
                title="Premium Clients Report 2022"
                category="Premium"
                stats={stats.premium}
              />
            </div>
            <p className="text-xl sm:text-3xl font-bold text-card-foreground">{formatCurrency(totals.premiumTotal)}</p>
          ...
          <div className="bg-gradient-to-br from-normal/10 to-normal/5 rounded-xl border border-normal/20 p-4 sm:p-6">
            <div className="flex items-center justify-between mb-2">
              ...
              <ExportDialog
                ...
                title="Normal Clients Report 2022"
                category="Normal"
                stats={stats.normal}
              />
            </div>
            <p className="text-xl sm:text-3xl font-bold text-card-foreground">{formatCurrency(totals.normalTotal)}</p>
          ...
          <div className="bg-gradient-to-br from-one-time/10 to-one-time/5 rounded-xl border border-one-time/20 p-4 sm:p-6">
            <div className="flex items-center justify-between mb-2">
              ...
              <ExportDialog
                ...
                title="One-Time Clients Report 2022"
                category="One-Time"
                stats={stats.oneTime}
              />
            </div>
            <p className="text-xl sm:text-3xl font-bold text-card-foreground">{formatCurrency(totals.oneTimeTotal)}</p>
        ...
        <Tabs defaultValue="premium" className="space-y-4 sm:space-y-6">
          ...
          <TabsContent value="monthly" className="mt-6 space-y-6">
            <div className="p-4 bg-primary/5 border border-primary/20 rounded-lg">
              <h3 className="font-semibold text-card-foreground flex items-center gap-2">
                ...
                Monthly Performance Overview (2022)
              </h3>
              <p className="text-sm text-muted-foreground mt-1">
                Track revenue, client activity, and segment performance across all months of 2022.
              </p>
            </div>
            <ChartsPanel />
          </TabsContent>
          ...
          <TabsContent value="all" className="mt-6">
            <ClientTable clients={[...premiumClients, ...normalClients, ...oneTimeClients]} title="All Clients (2022)" category="all" />
          </TabsContent>
          ...
          <TabsContent value="premium" className="mt-6 space-y-6">
            ...
            />
            <ClientTable clients={premiumClients} title="Premium Clients (2022)" category="premium" />
          </TabsContent>
          ...
          <TabsContent value="normal" className="mt-6 space-y-6">
            ...
            />
            <ClientTable clients={normalClients} title="Normal Clients (2022)" category="normal" />
          </TabsContent>
          ...
          <TabsContent value="one-time" className="mt-6 space-y-6">
            ...
            />
            <ClientTable clients={oneTimeClients} title="One-Time Clients (2022)" category="one-time" />
          </TabsContent>
        ...
        <div className="mt-8 sm:mt-12 bg-card rounded-xl border border-border p-4 sm:p-6 shadow-sm">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 sm:gap-6">
            ...
            <div>
              ...
              <p className="text-lg sm:text-2xl font-bold text-accent">{formatCurrency(totals.totalRevenue)}</p>
              <p className="text-xs text-muted-foreground">{totals.totalInvoices} invoices</p>
            </div>
          </div>
        </div>
      </main>
